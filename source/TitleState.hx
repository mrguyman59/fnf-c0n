	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('funnyMenu'), 100);
			}
		}
		...
		var bg:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, 0xFF330000);
		...
		if(!FlxG.save.data.psykaEasterEgg || !easterEggEnabled) {
			gfDance = new FlxSprite(FlxG.width * 0.31, FlxG.height * 0.07);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			switch (curBeat)
			{
				...
				// credTextShit.text += '\nNight';
				case 15:
					addMoreText('C0n'); // credTextShit.text += '\nFunkin';
